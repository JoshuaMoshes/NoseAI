...
class MLRModel(Model):
    ...
    def predict(self, X: np.ndarray | list[np.ndarray]) -> np.ndarray:
        ...
        return np.array([self._predict_one(x) for x in X_norm])
    ...
    def _predict_df(self, dataframe, target_column, dataset_name="dataset"):
        ...
        for index, x_test in enumerate(X_test):
            prediction = self._predict_one(x_test)
    ...
    def evaluate(self, dataframe, target_column, dataset_name="dataset"):
        start_time = time.time()
        predictions, y_true = self._predict_df(dataframe, target_column, dataset_name)
